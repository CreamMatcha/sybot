...
const MARKET_CHECK_INTERVAL_MS = 1 * 60 * 1000;    // 1분마다 시세 체크
...
// criteria.apiOpt     = API EtcOptions에 추가할 옵션 필터 (있으면 서버 필터링, 없으면 클라이언트 필터링)
//   스탯 필터: FirstOption=1, SecondOption=11 (힘/민첩/지능 공통), MinValue=하한
...
/**
 * @description 기준(criterion)에 맞는 API 파라미터로 상상 악세를 검색합니다.
 * - apiOpt가 있으면 세 번째 EtcOption을 추가해 서버에서 필터링
 * - apiQuality가 있으면 ItemGradeQuality 적용
 * - special(특수)이면 무공+/최생 두 번 요청 후 가격 오름차순으로 병합 (항상 페이지 1)
 */
function fetchAccItemsByCrit(accItem, crit, pageNo) {
    const url = "https://developer-lostark.game.onstove.com/auctions/items";
    const headers = {
        "accept": "application/json",
        "authorization": `bearer ${config.LOSTARK_API_KEY}`,
    ...
    const baseOpts = [
        { "FirstOption": 7, "SecondOption": accItem.opt1, "MinValue": accItem.v1, "MaxValue": accItem.v1 },
        { "FirstOption": 7, "SecondOption": accItem.opt2, "MinValue": accItem.v2, "MaxValue": accItem.v2 }
    ];

    function doFetch(etcOpts, quality, page) {
        const body = {
            "Sort": "BUY_PRICE",
            "CategoryCode": accItem.category,
            "ItemTier": 4,
            "ItemGrade": "고대",
            "ItemGradeQuality": quality || 67,
            "PageNo": page || 1,
            "SortCondition": "ASC",
            "EtcOptions": etcOpts
        };
        const r = fetchLostarkApi(url, headers, body);
        if (r.code !== 200) return { ok: false, items: [], code: r.code };
        const d = JSON.parse(r.body);
        return { ok: true, items: d.Items || [], code: 200 };
    }
    ...
    const etcOpts = crit.apiOpt ? baseOpts.concat([crit.apiOpt]) : baseOpts;
    return doFetch(etcOpts, crit.apiQuality, pageNo);
}
 ...
 * 각 기준별로 fetchAccItemsByCrit을 사용해 API에서 최대한 필터링 후, 거래횟수에 따라 분리 알림.
 */
            ...
            for (const c of item.criteria) {
                ...
                for (let page = 1; page <= maxPages; page++) {
                    ...
                    for (const it of result.items) {
                        const price = it.AuctionInfo.BuyPrice;
                ...
                if (bestZero) {
                    ...
                    const stateKey = item.key + ":" + c.key + ":0";
                    const state = accAlertState[stateKey];
                    if (!state || state.endDate !== endDate) {
...
// 조회할 단일 아이템 (상급 아비도스 융화 재료)
const MATERIAL_ABIDOS_ITEM = { CategoryCode: 50010, ItemName: "상급 아비도스" };
...
/**
 * @description 단일 아이템 검색 조건 목록을 조회해 시세 줄 배열로 변환합니다.
 * @param {object[]} queries CategoryCode/ItemName 등 검색 조건 목록
 * @returns {{ok: boolean, lines: string[]}} 401(인증 오류) 발생 시 ok:false 반환
 */
function fetchSectionLines(queries) {
    const lines = [];
    for (const query of queries) {
        ...
        if (!result.ok) {
            if (result.code === 401) return { ok: false, lines: [] };
            lines.push(`${label}: 조회 실패`);
            continue;
        }

        const item = result.items[0];
        lines.push(item ? formatMaterialLine(item) : `${label}: 매물 없음`);
    }
...
bot.addListener(Event.MESSAGE, (msg) => {
    ...
    if (content === ".시세봇" || content === ".시세명령어") {
        ...
        try {
            ...
            // 재련 재료
            const materialResult = fetchSectionLines(MATERIAL_ITEMS);
            if (!materialResult.ok) {
                msg.reply("⚠️ API 키가 만료되었거나 올바르지 않습니다.");
                isSuccess = false;
            } else {
                sections.push(materialResult.lines.join("\n"));
            }

            // 상급 아비도스 융화 재료
            if (isSuccess) {
                const abidosResult = fetchSectionLines([MATERIAL_ABIDOS_ITEM]);
                if (!abidosResult.ok) {
                    msg.reply("⚠️ API 키가 만료되었거나 올바르지 않습니다.");
                    isSuccess = false;
                } else {
                    sections.push(abidosResult.lines.join("\n"));
                }
            }
            ...
            // 단일 아이템 (에스더의 기운)
            if (isSuccess) {
                const soloResult = fetchSectionLines([MATERIAL_SOLO_ITEM]);
                if (!soloResult.ok) {
                    msg.reply("⚠️ API 키가 만료되었거나 올바르지 않습니다.");
                    isSuccess = false;
                } else {
                    sections.push(soloResult.lines.join("\n"));
                }
            }
    ...
    // .악세 <악세> <기준> → 해당 기준에 맞는 매물만 필터링해서 출력
        ...
        try {
            ...
            if (accKeyArg && accCritArg) {
                ...
                // 특수는 fetchAccItemsByCrit 내부에서 두 번 요청+병합하므로 페이지 1만 사용
                const MAX_QUERY_PAGES = crit.special ? 1 : 3;
                const matched = [];
                for (let page = 1; page <= MAX_QUERY_PAGES && matched.length < 3; page++) {
                    const result = fetchAccItemsByCrit(accItem, crit, page);
                    if (!result.ok) {
                        if (result.code === 401) {
                            msg.reply("⚠️ API 키가 만료되었거나 올바르지 않습니다.");
                            return;
                    ...
                    if (!crit.special && result.items.length < 10) break;
                }
